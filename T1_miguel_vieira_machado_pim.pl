...
jogadores_contemporaneos(Nome1, Nome2) :-
    distinct([Nome1,Nome2], (jogadores(Nome1, _, DataNascimento1, _, _, _, _, _, _),
...
?- jogadores_contemporaneos("Hoy Phallin", "Zé Rafael").
...
jogadores_parceiros(Nome1, Nome2) :-
    distinct([Nome1,Nome2], (jogadores(Nome1, _, _, Clube, _, _, _, _, _),
...
/** <examples> 
?- jogadores_parceiros(Nome1, Nome2).
?- jogadores_parceiros("Reung Bunheing", Nome2).
?- jogadores_parceiros("Reung Bunheing", "Sa Ty").
?- jogadores_parceiros("Paul Garita", "Marius Constantin").
...
jogadores_super_parceiros(Nome1, Nome2, Pais) :-
    distinct([Nome1,Pais,Nome2], (jogadores(Nome1, Pais, _, Clube, Posicao1, _, _, _, _),
...
/** <examples> 
?- jogadores_super_parceiros(Nome1, Nome2, Pais).
?- jogadores_super_parceiros(Nome1, Nome2, "Brazil").
?- jogadores_super_parceiros("Chhom Pisa", "Lim Pisoth", "Cambodia").
...
centro_avante_goleador(Nome, Gols) :-
    distinct([Nome,Gols], (jogadores(Nome, _, _, _, "Forward (association football)", _, NumeroCamisa, Gols, _),
    NumeroCamisa =:= 9,
    Gols > 50)).
...
?- centro_avante_goleador("Nahikari García", 104).
...
jogadores_concorrentes(Nome1, Nome2, Posicao, Clube) :-
    distinct([Nome1,Clube,Posicao,Nome2], (jogadores(Nome1, _, _, Clube, Posicao, _, _, _, _),
...
?- jogadores_concorrentes("Rodriguinho (footballer, born 2001)", "Claudinho (footballer, born 2000)", "Forward (association football)", "Associação Chapecoense de Futebol").
...
jogadores_rivais(Nome1, Nome2, Liga) :- 
    distinct([Nome1,Liga,Nome2], (jogadores(Nome1, _, _, Clube1, _, _, _, _, Liga),
...
goleiro_bom(Nome, Altura, Gols) :- 
    distinct([Nome,Altura,Gols], (jogadores(Nome, _, _, _, "Goalkeeper (association football)", Altura, _, Gols, _),
    Altura >= 1.90,
    Gols >= 1)).
...
jogadores_camisa_10_da_shoppe(Nome, Gols, NumeroCamisa) :- 
    distinct([Nome,NumeroCamisa,Gols], (jogadores(Nome, _, _, _, _, _, NumeroCamisa, Gols, _),
    Gols =:= 0,
    NumeroCamisa =:= 10)).
...
/** <examples>
?- jogadores_camisa_10_da_shoppe(Nome, Gols, NumeroCamisa).
?- jogadores_camisa_10_da_shoppe("Paulo Henrique Ganso", Gols, NumeroCamisa).
?- jogadores_camisa_10_da_shoppe("Stiven Mendoza", 0, 10).
